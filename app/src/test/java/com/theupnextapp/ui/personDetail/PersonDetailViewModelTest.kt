...
class PersonDetailViewModelTest {
    ...
    @Test
    fun `onCreditClicked sets navigateToShowDetail on successful lookup`() = runTest {
        val testImdbId = "tt123456"
        val testTitle = "Fake Show"
        val testTraktId = 1234
        
        // Arrange: Fake an id resolution
        val mockResponse = mock<NetworkTvMazeShowLookupResponse> {
    ...
    @Test
    fun `onCreditClicked sets errorMessage on lookup failure`() = runTest {
    ...
    @Test
    fun `clearErrorMessage successfully unsets the active error state`() = runTest {
